...
DEFAULTS = {
    ...
    "gate_header": {
        # the gate index (bbx gate-index; S6 step 3, R63, R68; D84): neutral where bbh's are its lineage's literals (D85)
        "title_sep_regex": r"\s*[—–-]+\s*",       # between `<name>.sh` and the claim on line 2
        "session_regex": r"\b(20\d\d-\d\d-\d\d)",  # `since` = the first match in the header: a date
        "duration_regex": r"~\s*(\d+(?:\.\d+)?)\s*(min|s\b|sec|h\b|hours?)",   # the runtime a header quotes
        "index_out": "docs/gate_index.md",         # the GENERATED index
        "families_tsv": "tests/gate_index.tsv",    # the family file, read by its extension: .tsv, or .toml (R68)
        "families": [],                            # [name, description] in order; none is implied
        "kinds": [],                               # name prefix -> kind; with none, every gate is "test"
        "needs_instruments": [],                   # header keyword (lowercased) -> the instrument named in `needs`
        "needs_build_regex": r"(?!)",              # a header matching this needs "a build dir"; this pattern matches nothing
        "index_preamble": ["# The gate index (GENERATED)", ""],   # the index's opening lines, the consumer's prose
        "columns": [],                             # further columns after bbh's five: "controls", "blind spots" (R68)
    },
}
...
KINDS = {
    "frame-driven": {
        ...
        # bbh's [gate_header] defaults verbatim (lib/py/bbh/config.py at 10a82d2; D85) for every key whose value differs
        # from the kind-blind one (D84): the lineage's literals, kept under the kind's name so a bbh config resolves here
        # to bbh's values and F19a can diff the gate index
        "gate_header": {
            "session_regex": r"\b(14z-\d+[a-z]?(?:\s*\(\d+\))?|M[0-9][a-z]?\b|session \d+[a-z]?|20\d\d-\d\d-\d\d)",
            "index_out": "docs/project/gate_index.md",
            ...
            "needs_build_regex": r"build/\w|build dir|builddir|<build>",
