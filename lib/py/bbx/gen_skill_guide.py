...
"""gen_skill_guide.py — GENERATE the human GUIDE.md beside a portable SKILL.md (H10).
  ...
  bbx skill-guide [--config bbx.toml] [--root DIR] [--check] [--prefix PFX ...]
...
Lifted into BBX from bbh `lib/py/bbh/gen_skill_guide.py` at `10a82d2` (slice S5 step 1,
2026-09-14; rulings R51, R56): the logic and every printed message unchanged but
one: the stale line names `bbx skill-guide` as the command to run, and F18 reads it as
`bbh skill-guide` before the diff; the config is read through BBX's layers (`$BBX_CONFIG`; the
...
def main():
    ...
    ap.add_argument("--config", help="the consumer's config (else $BBX_CONFIG)")
        ...
        if args.check:
            ...
            if have != text:
                ...
                print(f"  FAIL  {out_path.relative_to(root)} is STALE against SKILL.md + the docs "
                      f"— regenerate with bbx skill-guide --prefix {p}")
            else:
                n = text.count("\n**[")
