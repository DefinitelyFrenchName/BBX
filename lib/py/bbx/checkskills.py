...
"""checkskills.py — lock a consumer's SKILLS to the docs they distil (H10).
  ...
  bbx check-skills [--config bbx.toml] [--root DIR] [-v] [--no-selftest]
...
paragraph of the docs it distils, and four things are asserted:
  ...
  4. CROSS-REFERENCES RESOLVE.  A plain `[PFX-N]` naming another configured
     ...
     the bracket tokens instead (bbh `docs/config.md` [skills]).
...
Lifted into BBX from bbh `lib/py/bbh/checkskills.py` at `10a82d2` (slice S5 step 1,
2026-09-14; rulings R51, R56): the logic and every printed message unchanged but
none (this file prints no command name); the config is read through BBX's layers (`$BBX_CONFIG`; the
