...
a lib it SOURCES does, followed to [tier].source_depth. Lifted from bbh
lib/py/bbh/tier.py (S1, 2026-09-09); its lineage is VampireSaved's
run_all_static.sh, where the check had to become transitive after two gates
reached MAME only through a sourced lib and were reported static; one of them
then ran for 208 s inside a chain advertised as emulator-free.

One generalization: an EMPTY pattern list means nothing reaches an
instrument. (A regex alternation of zero patterns is the empty regex, which
matches everything — every gate would have been INSTRUMENT and the anti-
orphan report silent. bbh never had an empty list; the kind-blind default
is one.)
...
class Tier:
    def __init__(self, cfg, root):
        self.root = root
        pats = list(C.get(cfg, "tier.patterns"))
        self.emu = re.compile("|".join(f"(?:{p})" for p in pats)) if pats else None
