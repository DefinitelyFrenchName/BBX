#!/usr/bin/env python3
"""propose_temporal.py (bbh: describe_masked_shape.py) — the measured shape of a masked divergence, and a
PROPOSED expectation line in the ratified vocabulary.

Thresholds come from bbx.thresholds — the SINGLE declaration shared with
...
Usage: python3 -m bbx.propose_temporal <base.log> <new.log> [--basis <set>/<basis>]

Lifted VERBATIM from bbh lib/py/bbh/describe_masked_shape.py at f675710 (BBX slice S2 step 1, bbx-3, 2026-09-10): every printed line is
frozen verdict text (abstraction C4) and fidelity F17 diffs it against bbh's over the same logs. bbh -> bbx
in module names and the config variable only; the lineage's incident citations stay as history lines.
"""
