...
Lifted from bbh lib/py/bbh/toml_subset.py (slice S1, 2026-09-09), whose
lineage is VampireSaved's tools/_minitoml.py: a fallback for Python < 3.11
that learned, at cost, to REFUSE the constructs on which it and tomllib
disagree (a nested row parsed there as a flat orphan key and never applied,
while a >= 3.11 host would have applied it — GitHub #42 of that project).
Same accept set, same refuse set, same messages: fidelity F13 diffs a runner
that reads a config through this module against bbh's, and any drift here
would show there. BBX adds nothing to the grammar (a `[project].kind` key is
an ordinary key).
...
gates/config.sh asserts that every refused example is refused and, on a host
with tomllib, that the two parsers agree on every accepted example.
"""
import re
