...
it to an expectation set (the auto-detecting runner's dispatch): the SUBJECT
...
VampireSaved's tools/build_fingerprint.py. One change of shape: the settings
resolve through the config layers (consumer → kind profile → kind-blind,
config.py), so the lineage's literals live in the `frame-driven` profile and
a kindless config reads them as bbh does (fidelity F14f), while the kind-
blind default is `file-sha1` — the artifact is one file (D16). Every printed
line is the lineage's text.
...
are added only at freeze time, as a build decision. The identity is computed
from the artifact, never from what the artifact says about itself
(CLAUDE.md §3.4).
...
_KEYS = ("kind", "program_member_regex", "parent_sets", "region_rules", "region_default",
         "file_pattern", "program_command", "wholeset_command")


def settings(config_path=None):
    """The [fingerprint] section plus the suite's registry / default set,
    from --config, else $BBX_CONFIG, else the config layers with no consumer
    (the kind profile named by the default kind, then the kind-blind values)."""
    path = config_path or os.environ.get("BBX_CONFIG")
    cfg, root = ({}, None)
    if path and os.path.isfile(path):
        cfg = C.load(path)
        root = C.root_of(path, cfg)
    vals = {k: C.get(cfg, "fingerprint." + k) for k in _KEYS}
    reg = C.get(cfg, "suite.registry")
    vals["registry"] = os.path.join(root, reg) if root else reg
    vals["default_set"] = C.get(cfg, "suite.default_set")
    return vals
...
def main(argv=None):
    ...
    ap.add_argument("--config", default=None, help="the consumer config (else $BBX_CONFIG, else the default kind's literals)")
