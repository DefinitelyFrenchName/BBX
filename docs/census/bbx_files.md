...
**Measured at harness identity `f189534d8e9ef9496c4df90f651c66d38a3e54f7`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 47 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 33 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.
...
| A2 | registered gates (both tiers) | 33 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 33 | §B.1, one row each |
...
| A8 | gates whose derived kind is `K` | 16 | §B.1 |
...
| A12 | files: frame-driven only | 10 | §B.2 |
...
|---|---|---|
| `bin/bbx` | 6 | adapters band fidelity_bbh_s5 file_census_tool json_schema set_schema |
| `bin/bbx-run-suite` | 5 | adapters cli_suite docset_suite fidelity_bbh_s2 suite |
| `lib/py/bbx/__init__.py` | 30 | adapters band classify cli_driver cli_fixture cli_suite close_sweeps compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 file_census_tool fingerprint json_schema provenance readout registry_complete set_schema static_runner suite sweep_runner temporal thresholds tier |
| `lib/py/bbx/config.py` | 27 | adapters band classify cli_fixture cli_suite compare_dispatch config controls docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 file_census_tool fingerprint json_schema provenance readout registry_complete set_schema static_runner suite sweep_runner temporal thresholds tier |
...
| `lib/py/bbx/checkskills.py` | 1 | fidelity_bbh_s5 |
...
| `lib/py/bbx/gen_skill_guide.py` | 1 | fidelity_bbh_s5 |
