...
**Measured at harness identity `c26e5a94b344ea01ca960cd4e7ec228c621aec76`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 45 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 32 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.
...
| A2 | registered gates (both tiers) | 32 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 32 | §B.1, one row each |
...
| `lib/py/bbx/__init__.py` | 29 | adapters band classify cli_driver cli_fixture cli_suite close_sweeps compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 file_census_tool fingerprint json_schema provenance readout registry_complete set_schema static_runner suite sweep_runner temporal thresholds tier |
| `lib/py/bbx/config.py` | 26 | adapters band classify cli_fixture cli_suite compare_dispatch config controls docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 file_census_tool fingerprint json_schema provenance readout registry_complete set_schema static_runner suite sweep_runner temporal thresholds tier |
