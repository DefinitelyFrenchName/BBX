...
**Measured at harness identity `016163742714ae1a733be70bea6b6c0f2b433d2d`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 53 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 38 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 53 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
| A2 | registered gates (both tiers) | 38 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 38 | §B.1, one row each |
...
| A8 | gates whose derived kind is `K` | 20 | §B.1 |
...
| A16 | files: kernel only | 8 | §B.2 |
| A17 | files: REACHED BY NO GATE | 0 | §B.2 |
| A18 | files executed by exactly one gate | 10 | §B.2, the `gates` column |
...
| `lib/py/bbx/__init__.py` | 35 | adapters band classify cli_driver cli_fixture cli_suite close_sweeps compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool fingerprint gate_index json_schema provenance readout registers registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier trap_lint |
...
| `lib/py/bbx/controls.py` | 7 | cli_suite close_sweeps controls docset_suite fidelity_bbh_s6 gate_index readout |
...
| `lib/py/bbx/toml_subset.py` | 33 | adapters band classify cli_driver cli_fixture cli_suite compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool fingerprint gate_index json_schema provenance readout registers registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier |
...
| `lib/py/bbx/sha1.py` | 4 | adapters cli_suite docset_suite registers |
...
| `lib/py/bbx/gen_gate_index.py` | 2 | fidelity_bbh_s6 gate_index |
