...
**Measured at harness identity `5059aa730eb59e148cdf06909b896b604f3977f0`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 49 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 34 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.
