...
**Measured at harness identity `fe9928e94eac71039a4d6001c0c09a45a5618881`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 44 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 30 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below.
...
| `sweep_runner` | portable | K | 8 | PASS | 92 |
...
| `set_schema` | portable | D | 15 | PASS | 10 |
| `docset_suite` | portable | D | 21 | PASS | 80 |
| `cli_fixture` | portable | C | 9 | PASS | 3 |
| `cli_driver` | portable | C | 5 | PASS | 14 |
| `band` | portable | C | 13 | PASS | 6 |
| `json_schema` | portable | C | 15 | PASS | 9 |
| `cli_suite` | portable | C | 22 | PASS | 128 |
| `adapters` | portable | C | 24 | PASS | 132 |
| `file_census_tool` | portable | K | 8 | PASS | 13 |
| `census_recount` | static | K | 1 | PASS | 66 |
| `fidelity_bbh` | static | F | 10 | PASS | 45 |
| `fidelity_bbh_s2` | static | F | 17 | PASS | 93 |
| `suite` | static | F | 15 | PASS | 85 |
