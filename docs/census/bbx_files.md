...
**Measured at harness identity `670d71385e8a28b0236e42f5c2a0cb50a461eb55`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 49 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 34 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 49 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
| A2 | registered gates (both tiers) | 34 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 34 | §B.1, one row each |
...
| A8 | gates whose derived kind is `K` | 17 | §B.1 |
...
| A16 | files: kernel only | 6 | §B.2 |
| A17 | files: REACHED BY NO GATE | 0 | §B.2 |
| A18 | files executed by exactly one gate | 8 | §B.2, the `gates` column |
