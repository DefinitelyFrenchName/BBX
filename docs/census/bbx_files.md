...
**Measured at harness identity `e1c42cfe6c125077d9ded94f1261ceb3259769b4`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 44 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 30 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below.
...
| `fidelity_bbh` | static | F | 10 | PASS |
| `fidelity_bbh_s2` | static | F | 17 | PASS |
| `suite` | static | F | 15 | PASS |
