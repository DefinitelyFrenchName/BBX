...
**Measured at harness identity `47ec729e1591dee58144306bcb1ae01e8355eb2c`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 44 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 30 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 44 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
| A2 | registered gates (both tiers) | 30 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 30 | §B.1, one row each |
| A4 | harness files instrumented (one trace line each) | 15 | §C: every `bin/*`, `drivers/*.sh`, `lib/sh/*.sh` plus `lib/py/sitecustomize.py` |
| A5 | gates whose derived kind is `C` | 6 | §B.1 |
| A6 | gates whose derived kind is `D` | 4 | §B.1 |
| A7 | gates whose derived kind is `F` | 6 | §B.1 |
| A8 | gates whose derived kind is `K` | 14 | §B.1 |
...
| A12 | files: frame-driven only | 7 | §B.2 |
...
| `controls` | portable | K | 9 | PASS | 7 |
| `sweep_runner` | portable | K | 8 | PASS | 93 |
| `fingerprint` | portable | K | 4 | PASS | 2 |
| `rulings_shape` | portable | K | 1 | PASS | 0 |
| `readout` | portable | K | 11 | PASS | 4 |
| `close_sweeps` | portable | K | 2 | PASS | 5 |
...
| `provenance` | portable | K | 4 | PASS | 1 |
| `docset_fixture` | portable | D | 9 | PASS | 1 |
| `docset_driver` | portable | D | 5 | PASS | 5 |
| `set_schema` | portable | D | 15 | PASS | 11 |
| `docset_suite` | portable | D | 21 | PASS | 81 |
| `cli_fixture` | portable | C | 9 | PASS | 3 |
| `cli_driver` | portable | C | 5 | PASS | 15 |
| `band` | portable | C | 13 | PASS | 6 |
| `json_schema` | portable | C | 15 | PASS | 10 |
| `cli_suite` | portable | C | 22 | PASS | 132 |
| `adapters` | portable | C | 24 | PASS | 133 |
| `file_census_tool` | portable | K | 8 | PASS | 12 |
| `census_recount` | static | K | 1 | PASS | 68 |
| `fidelity_bbh` | static | F | 10 | PASS | 45 |
| `fidelity_bbh_s2` | static | F | 17 | PASS | 97 |
| `suite` | static | F | 15 | PASS | 87 |
