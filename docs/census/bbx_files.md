...
**Measured at harness identity `bc53b3b479db7c3af3688f4ae90a3bd29d3c51b4`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 51 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 35 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 51 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
| A2 | registered gates (both tiers) | 35 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 35 | §B.1, one row each |
...
| A7 | gates whose derived kind is `F` | 7 | §B.1 |
| A8 | gates whose derived kind is `K` | 18 | §B.1 |
| A9 | files: shared (frame AND document-set AND command-line) | 13 | §B.2 |
| A10 | files: frame-driven and command-line | 4 | §B.2 |
| A11 | files: document-set and command-line | 9 | §B.2 |
| A12 | files: frame-driven only | 8 | §B.2 |
| A13 | files: frame-driven only (+kernel) | 3 | §B.2 |
...
**shared (frame AND document-set AND command-line): 13**

| file | gates | executed by |
|---|---|---|
| `bin/bbx` | 7 | adapters band fidelity_bbh_s5 file_census_tool json_schema set_schema skills |
| `bin/bbx-run-suite` | 5 | adapters cli_suite docset_suite fidelity_bbh_s2 suite |
| `lib/py/bbx/__init__.py` | 32 | adapters band classify cli_driver cli_fixture cli_suite close_sweeps compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 file_census_tool fingerprint json_schema provenance readout registers registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier |
| `lib/py/bbx/config.py` | 28 | adapters band classify cli_fixture cli_suite compare_dispatch config controls docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 file_census_tool fingerprint json_schema provenance readout registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier |
...
**document-set and command-line: 9**
...
| `lib/py/bbx/controls.py` | 5 | cli_suite close_sweeps controls docset_suite readout |
...
| `lib/sh/baseline.sh` | 4 | fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 suite |

**frame-driven only (+kernel): 3**
