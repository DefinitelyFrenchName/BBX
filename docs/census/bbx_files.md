...
**Measured at harness identity `da6bb9deb618675fbb9264bf17ff01678d06021e`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 48 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 34 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 48 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
...
| A16 | files: kernel only | 5 | §B.2 |
| A17 | files: REACHED BY NO GATE | 0 | §B.2 |
| A18 | files executed by exactly one gate | 7 | §B.2, the `gates` column |
