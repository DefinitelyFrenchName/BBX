...
**Measured at harness identity `68c94b57455263f0b7bdaf87e7289e1756ea6454`** (R38's whole-set key over `bin`, `lib`, `drivers`, `gates` — NOT `HEAD`, so a commit that only rewrites this document leaves it checkable) over 55 tracked files under `lib/`, `bin/`, `drivers/` (`*.md` excluded) and 39 gates, each run ONCE in the shadow with a fresh trace. `bbx file-census --check` fails on any line below. **1 gate(s) SKIPPED in the shadow** and so under-report what they reach: `census_register`.

## A. Counts

| id | dimension | count | command / derivation |
|---|---|---|---|
| A1 | tracked files under lib/, bin/, drivers/ (README.md excluded) — the universe | 55 | `git ls-files lib bin drivers | grep -vc '\.md$'` |
| A2 | registered gates (both tiers) | 39 | `grep -hv '^#' gates/portable.txt gates/static.txt | wc -l` |
| A3 | gates run in the shadow, each once, alone, with a fresh trace | 39 | §B.1, one row each |
...
| A7 | gates whose derived kind is `F` | 8 | §B.1 |
| A8 | gates whose derived kind is `K` | 21 | §B.1 |
| A9 | files: shared (frame AND document-set AND command-line) | 14 | §B.2 |
| A10 | files: frame-driven and command-line | 4 | §B.2 |
| A11 | files: document-set and command-line | 8 | §B.2 |
| A12 | files: frame-driven only | 8 | §B.2 |
| A13 | files: frame-driven only (+kernel) | 5 | §B.2 |
| A14 | files: document-set only | 1 | §B.2 |
| A15 | files: command-line only | 5 | §B.2 |
| A16 | files: kernel only | 10 | §B.2 |
| A17 | files: REACHED BY NO GATE | 0 | §B.2 |
| A18 | files executed by exactly one gate | 12 | §B.2, the `gates` column |
...
| `readout` | portable | K | 12 | PASS |
| `close_sweeps` | portable | K | 3 | PASS |
...
| `file_census_tool` | portable | K | 9 | PASS |
| `census_register` | portable | K | 0 | SKIP |
| `registry_complete` | portable | K | 4 | PASS |
| `skills` | portable | K | 8 | PASS |
| `registers` | portable | K | 7 | PASS |
| `trap_lint` | portable | K | 2 | PASS |
| `gate_index` | portable | K | 5 | PASS |
| `references` | portable | K | 4 | PASS |
| `census_recount` | static | K | 1 | PASS |
| `fidelity_bbh` | static | F | 11 | PASS |
| `fidelity_bbh_s2` | static | F | 18 | PASS |
| `fidelity_bbh_s5` | static | F | 8 | PASS |
| `fidelity_bbh_s6` | static | F | 8 | PASS |
| `suite` | static | F | 16 | PASS |

### B.2 Files, by category, with every gate that executed them

**shared (frame AND document-set AND command-line): 14**

| file | gates | executed by |
|---|---|---|
| `bin/bbx` | 8 | adapters band fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool json_schema set_schema skills |
| `bin/bbx-run-suite` | 5 | adapters cli_suite docset_suite fidelity_bbh_s2 suite |
| `lib/py/bbx/__init__.py` | 36 | adapters band classify cli_driver cli_fixture cli_suite close_sweeps compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool fingerprint gate_index json_schema provenance readout references registers registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier trap_lint |
| `lib/py/bbx/config.py` | 30 | adapters band classify cli_fixture cli_suite compare_dispatch config controls docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool fingerprint gate_index json_schema provenance readout registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier |
| `lib/py/bbx/controls.py` | 8 | cli_suite close_sweeps controls docset_suite fidelity_bbh_s6 gate_index readout registers |
...
| `lib/py/bbx/fingerprint.py` | 11 | adapters cli_fixture cli_suite docset_fixture docset_suite fidelity_bbh fidelity_bbh_s2 file_census_tool fingerprint suite sweep_runner |
...
| `lib/py/bbx/recount.py` | 10 | adapters band census_recount cli_driver cli_fixture cli_suite docset_suite fidelity_bbh_s5 json_schema set_schema |
| `lib/py/bbx/toml_subset.py` | 34 | adapters band classify cli_driver cli_fixture cli_suite compare_dispatch config controls docset_driver docset_fixture docset_suite expectation_kinds fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 file_census_tool fingerprint gate_index json_schema provenance readout references registers registry_complete set_schema skills static_runner suite sweep_runner temporal thresholds tier |
...
| `lib/py/bbx/tier.py` | 8 | adapters controls fidelity_bbh readout registry_complete static_runner sweep_runner tier |
...
**document-set and command-line: 8**
...
| `lib/py/bbx/sha1.py` | 5 | adapters cli_suite docset_suite references registers |
...
| `lib/sh/baseline.sh` | 5 | fidelity_bbh fidelity_bbh_s2 fidelity_bbh_s5 fidelity_bbh_s6 suite |

**frame-driven only (+kernel): 5**

| file | gates | executed by |
|---|---|---|
| `bin/bbx-run-sweep` | 2 | fidelity_bbh sweep_runner |
| `lib/py/bbx/checkskills.py` | 2 | fidelity_bbh_s5 skills |
| `lib/py/bbx/demand_after_trap.py` | 2 | fidelity_bbh_s6 trap_lint |
| `lib/py/bbx/gen_gate_index.py` | 3 | fidelity_bbh_s6 gate_index readout |
| `lib/py/bbx/gen_skill_guide.py` | 2 | fidelity_bbh_s5 skills |
...
| `lib/py/bbx/defaults.py` | 1 | registers |
| `lib/py/bbx/documents.py` | 1 | registers |
| `lib/py/bbx/file_census.py` | 1 | file_census_tool |
| `lib/py/bbx/gen_skill.py` | 1 | skills |
| `lib/py/bbx/reanchors.py` | 1 | skills |
| `lib/py/bbx/references.py` | 1 | references |
| `lib/py/bbx/rot.py` | 1 | registers |
