...
- **E3 — PROVENANCE CLASS.** Every frozen file has a row in a register naming
  ...
  complete both ways `[BBH-52]` `[BBH-53]`. The closed vocabulary is ruling
  R11; evidence is upgraded, never silently downgraded (CLAUDE.md §3.3).
...
- **G2 — HEADER API.** Line 2 is the claim `[BBH-18]`; the header names the
  ...
  `[BBH-20]`.
- **G3 — CONTROLS ARE DECLARED, AND THE DECLARATION IS READ.** *New; ruling
...
- **G4 — REGISTERED OR NOT RUN.** Every gate is in a registry; the anti-orphan
  ...
  transitively `[BBH-23]`.
- **G5 — SETUP-FAIL.** *Proposed; ruling R13.* A gate whose fixture did not
...
## 10. What this abstraction does NOT decide

Directory layout (contracts only); language beyond ruling R3; whether the
readout is text, TSV or HTML; the closed provenance vocabulary (R11); whether
SETUP-FAIL is reported (R13); the fixture subjects' content
(`docs/generality.md`); anything about performance.
