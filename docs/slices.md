...
| **S3 — the document-set kind** (plan: `docs/plans/S3.md`, bbx-6, 2026-09-10 — STOPPED; R31–R34 answered the same day, step 1 may open) | kind profile (S1), the doc-set driver (D1–D5), the claim-inventory and coverage kinds, set/multiset and schema families, the fixture `fixture/docset/` with its `--check` generator, the doc-set gate battery | — (no ancestor; proof by fixture) | the edited document; the shifted artifact; the planted wrong claim; the paraphrase; the unbindable claim | coverage as a number; "prose, reasoning and causal claims are not asserted" | answered | 3 (= 1.5 × 2) |
