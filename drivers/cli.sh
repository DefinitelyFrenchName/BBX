...
# NOT-ASSERTED: what the HOST records about a signal death: a subject that dies by a FAULT signal (SIGABRT, SIGSEGV, SIGILL, SIGBUS, SIGTRAP, SIGFPE, SIGSYS) makes the operating system write a crash report OUTSIDE the sandbox (on macOS ~/Library/Logs/DiagnosticReports) which this driver neither creates nor removes — the sandbox's removal does not reach it, and the fixture's own control dies by SIGKILL for exactly that reason (R42, G30)
