#!/bin/sh
# unittest.sh — THE UNITTEST ADAPTER: drivers/README.md's contract over a PYTHON UNITTEST PACKAGE — a
# directory of test modules run by `python3 -m unittest -v`, whose per-case verdicts are the observation
...
#
# Usage: CLI_PATH="<dir>[;<dir>]" drivers/unittest.sh <set> <scenario.cli> <out.log> [sandbox]
#   env CLI_PATH      the search path: the first directory holding a DIRECTORY named <set> wins (the package directory)
...
set -eu
SET="${1:?usage: unittest.sh <set> <scenario.cli> <out.log> [sandbox]}"
...
for v in MASK_RANGES DUMPS POKES SNAP_FRAMES VIDEO_OUT INPUT_OUT TAIL_FRAMES INPUT_INJECT_TEST NO_INPUT_CHECK; do
    eval "_x=\${$v:-}"
    [ -z "$_x" ] || { echo "REFUSED: drivers/unittest.sh cannot honour $v (the frame-driven family: a framework has no frames and no RAM)"; exit 3; }
done
for v in GUARD_DEBUG GUARD_PROBE GUARD_PROBE_COND GUARD_TRACE GUARD_PC_LOG GUARD_BREAK GUARD_MATCH GUARD_FORCE CRASH_VECTORS CODE_RANGES; do
    eval "_x=\${$v:-}"
    [ -z "$_x" ] || { echo "REFUSED: drivers/unittest.sh cannot honour $v (a guarded driver's variable; a framework has no debugger)"; exit 3; }
done
for v in DOCSET_NONDET DOCSET_VIEW DOCSET_FORMS; do
    eval "_x=\${$v:-}"
    [ -z "$_x" ] || { echo "REFUSED: drivers/unittest.sh cannot honour $v (the document-set family: a framework has no claim forms)"; exit 3; }
done
[ -z "${CLI_KEEP_ENV:-}" ] || { echo "REFUSED: drivers/unittest.sh cannot honour CLI_KEEP_ENV (the subject sees D6's hermetic set plus the scenario's [env] table and nothing from the caller, D52)"; exit 3; }
TIMEOUT="${CLI_TIMEOUT:-60}"
case "$TIMEOUT" in
    ''|*[!0-9]*|0) echo "REFUSED: drivers/unittest.sh cannot honour CLI_TIMEOUT='$TIMEOUT' (a positive number of seconds, D51)"; exit 3 ;;
esac

[ -f "$SCEN" ] || { echo "unittest.sh: scenario '$SCEN' is not a file"; exit 1; }
...
python3 -m bbx.adapters unittest "$SETDIR" "$SCEN" "$OUT" "$SANDBOX" $_nd --timeout "$TIMEOUT" && _st=0 || _st=$?
[ "$FRESH" = 0 ] || rm -rf "$SANDBOX"
[ "$_st" = 0 ] || exit "$_st"
grep -q "^END " "$OUT" || { echo "unittest.sh: the run did not complete (no END line)"; exit 1; }
exit 0
