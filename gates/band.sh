...
# band row, "C1, C2", §5's ruling-less-class row; R36; D48, D53, D55; the band view handed by the caller, D57). The driver runs on the TREE's fixture (read-only)
...
expect "04_band band" 0 "PASS band (1 field(s), every value inside its band)" pass -- compare_check "$E" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
...
view "$want" "$W/same"; expect "the same value through a view copy" 0 "PASS band (1 field(s), every value inside its band)" pass -- compare_check "$E" 04_band band - "" "$W/same.log" "$SC/04_band.cli" "$W/same.log.bands"
...
expect "a band spec whose class is not band" 1 "FAIL unknown tolerant-numeric class 'sortof'" unknown-class -- compare_check "$X" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
...
expect "no band view handed (the kinds loop's argument missing)" 1 "FAIL band: the tolerant-numeric family needs the band view (compare_check's trailing argument)" diverged -- compare_check "$E" 04_band band - "" "$L"
expect "an expectation file that is not there" 1 "FAIL band: $X/09_none.band is not a band spec (no such file)" diverged -- compare_check "$X" 09_none band - "" "$L" "$SC/09_none.cli" "$L.bands"
drv "$F/subject" "$SC/01_list.cli" "$W/01_list.log"; [ ! -f "$W/01_list.log.bands" ] || fail "01_list has a band view"
expect "a log with no band view (01_list declares no bands)" 1 "FAIL band: the band view $W/01_list.log.bands cannot be read (No such file or directory: no band field was observed — the scenario declares no bands, or stdout was not one JSON object)" diverged -- compare_check "$E" 04_band band - "" "$W/01_list.log" "$SC/04_band.cli" "$W/01_list.log.bands"
grep -v '^measured' "$E/04_band.band" > "$X/04_band.band"
expect "a band with no measured statement" 1 "FAIL band: $X/04_band.band is not a band spec ([spec] lacks measured: a band with no measurement is a knob (R36))" diverged -- compare_check "$X" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
sed 's/^min = 236$/min = 271/' "$E/04_band.band" > "$X/04_band.band"
expect "a band whose min is above its max" 1 "FAIL band: $X/04_band.band is not a band spec ([b1] min 271 is above max 270)" diverged -- compare_check "$X" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
sed 's/^max = 270$/max = 270.5/' "$E/04_band.band" > "$X/04_band.band"
expect "a fractional band (the subset refuses the float, D48)" 1 "FAIL band: $X/04_band.band is not a band spec (line 9: unsupported value syntax: '270.5')" diverged -- compare_check "$X" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
sed 's/^measured = \(.*\)$/measured = \1\nrulings = { seed = "R36" }/' "$E/04_band.band" > "$X/04_band.band"
expect "a rulings entry for a field no table freezes" 1 "FAIL band: $X/04_band.band is not a band spec ([spec].rulings names 'seed', which no [b<i>] table freezes)" diverged -- compare_check "$X" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands"
view '265.5' "$W/v1"; expect "a view value that is not an integer" 1 "FAIL band size: 265.5 is not an integer (the band is [236, 270])" diverged -- compare_check "$E" 04_band band - "" "$W/v1.log" "$SC/04_band.cli" "$W/v1.log.bands"
view '"265"' "$W/v2"; expect "a view value that is a string" 1 'FAIL band size: "265" is not an integer (the band is [236, 270])' diverged -- compare_check "$E" 04_band band - "" "$W/v2.log" "$SC/04_band.cli" "$W/v2.log.bands"
cp "$L" "$W/v3.log"; printf '1 size=265\nEND 2\n' > "$W/v3.log.bands"
expect "a view whose END disagrees" 1 "FAIL band: the band view $W/v3.log.bands is malformed (END 2 but 1 rows)" diverged -- compare_check "$E" 04_band band - "" "$W/v3.log" "$SC/04_band.cli" "$W/v3.log.bands"
cp "$L" "$W/v4.log"; printf '1 size=265\n' > "$W/v4.log.bands"
expect "a view with no END" 1 "FAIL band: the band view $W/v4.log.bands is malformed (no END line last)" diverged -- compare_check "$E" 04_band band - "" "$W/v4.log" "$SC/04_band.cli" "$W/v4.log.bands"
cp "$L" "$W/v5.log"; printf '1 size=265\n1 size=265\nEND 2\n' > "$W/v5.log.bands"
expect "a view whose indices do not run 1..k" 1 "FAIL band: the band view $W/v5.log.bands is malformed (line 2 is not \`2 <name>=<value>\`)" diverged -- compare_check "$E" 04_band band - "" "$W/v5.log" "$SC/04_band.cli" "$W/v5.log.bands"
...
if [ "$lo" != 236 ] || [ "$hi" != 270 ]; then fail "CONTROL DEAD: outside-the-band — the frozen band is [$lo, $hi], not the design's [236, 270]"; else
    view "$((lo - 1))" "$W/below"; a=0; try 1 "FAIL band size: 235 outside [236, 270]" diverged -- compare_check "$E" 04_band band - "" "$W/below.log" "$SC/04_band.cli" "$W/below.log.bands" && a=1
    view "$((hi + 1))" "$W/above"; b=0; try 1 "FAIL band size: 271 outside [236, 270]" diverged -- compare_check "$E" 04_band band - "" "$W/above.log" "$SC/04_band.cli" "$W/above.log.bands" && b=1
    view "$lo" "$W/atlo"; c=0; try 0 "PASS band (1 field(s), every value inside its band)" pass -- compare_check "$E" 04_band band - "" "$W/atlo.log" "$SC/04_band.cli" "$W/atlo.log.bands" && c=1
    view "$hi" "$W/athi"; d=0; try 0 "PASS band (1 field(s), every value inside its band)" pass -- compare_check "$E" 04_band band - "" "$W/athi.log" "$SC/04_band.cli" "$W/athi.log.bands" && d=1
...
if ! grep -q '^1 seed=4$' "$W/vi.log.bands" || ! grep -q '^\[b2\]$' "$I/04_band.band"; then fail "CONTROL DEAD: inventory-both-ways — a perturbation did not apply"; else
    a=0; try 1 "FAIL band: view field 'seed' is not frozen (a band nobody froze, BBX-13)" diverged -- compare_check "$E" 04_band band - "" "$W/vi.log" "$SC/04_band.cli" "$W/vi.log.bands" && a=1
    b=0; try 1 "FAIL band: frozen field 'seed' is not in the band view" diverged -- compare_check "$I" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands" && b=1
    c=0; try 0 "PASS band (2 field(s), every value inside its band)" pass -- compare_check "$I" 04_band band - "" "$W/vi.log" "$SC/04_band.cli" "$W/vi.log.bands" && [ "$(sed -n 2p "$W/out")" = "NOTE: band-fields 2" ] && c=1
...
if cmp -s "$W/wider.band" "$E/04_band.band" || ! grep -q '^rulings = { size = "R36" }$' "$W/ruled.band" || ! grep -q '^min = 240$' "$W/narrower.band"; then fail "CONTROL DEAD: ruling-less-class — a perturbation did not apply"; else
    ...
    a=0; try 1 "FAIL band size: 273 outside [236, 270]" diverged -- compare_check "$R" 04_band band - "" "$W/v273.log" "$SC/04_band.cli" "$W/v273.log.bands" && a=1
    ...
    d=0; try 0 "PASS band (1 field(s), every value inside its band)" pass -- compare_check "$R" 04_band band - "" "$W/v273.log" "$SC/04_band.cli" "$W/v273.log.bands" && d=1
    ...
    f=0; try 1 "FAIL band size: 238 outside [240, 270]" diverged -- compare_check "$R" 04_band band - "" "$W/v238.log" "$SC/04_band.cli" "$W/v238.log.bands" && f=1
...
if ! grep -q 'PASS band \[' "$SH/lib/py/bbx/compare_band.py"; then fail "CONTROL DEAD: verdict-text — the shadow was not perturbed (sed matched nothing)"; else
    o="$(env BBX_HOME="$SH" PYTHONPATH="$SH/lib/py" sh -c '. "$BBX_HOME/lib/sh/compare.sh"; compare_check "$@"' _ "$E" 04_band band - "" "$L" "$SC/04_band.cli" "$L.bands" 2>&1 | head -1)" && s=0 || s=$?
