...
| **slice S4's plan** (the ancestors measured, the design contract by contract, the fixture `fixture/fakecli/`, eighteen controls, the adapters, the self subject, the file census as a gate, rulings R35–R40) | `docs/plans/S4.md` — bbx-12; R35–R40 answered 2026-09-10 (R40 with the TSV caveat); **step 1 built (bbx-13)**; §3/§4 corrected at bbx-13 before step 1 landed (X14–X19, BBX-19); **step 2 is next** |
| **the command-line kind (S4 step 1)** | the profile `lib/py/bbx/config.py` KINDS `command-line` (D45); `[suite].scenario_ext = "cli"` (D34); the token vocabulary `lib/py/bbx/cli.py` (D47 — the ONE writer of every token; the driver's core grows here at step 2); the fixture `fixture/fakecli/` (BBX's third consumer: `bbx.toml`, `mkfakecli.py --check` — the design's predicates on the tree's tool, regenerate-and-diff, and the TOOL-CHECK that runs `subject/fakecli.py` for every scenario against the design; `scenarios/*.cli` (D46), `expected/fixture/` with `truth` + `logs/`, `unordered`, `schema` json (D49), `band` (D48), the registry and the 22-row register); `gates/cli_fixture.sh` (4 controls) |
...
| BBX's gates and registries | `gates/*.sh` (24: 20 portable incl. `temporal`, `thresholds`, `compare_dispatch`, `expectation_kinds`, `provenance`, `docset_fixture`, `docset_driver`, `set_schema`, `docset_suite`, `cli_fixture`; 4 static incl. `fidelity_bbh_s2` (F12, F16, F17) and `suite`), `gates/portable.txt`, `gates/static.txt`, `gates/sweep.tsv` (empty) — run with `BBX_BBH_HOME=~/Developer/blackbox-harness bin/bbx selftest` (~6 min on a loaded host: the two S2 static gates are ~100 s and ~60 s, `docset_suite` ~55 s) |
...
bin/bbx selftest` (~6–7 min on this host) is GREEN twice at one HEAD: 24 gates, 82/82 controls (`build/selftest_20260910T171631Z`, `build/selftest_20260910T172316Z`) (the bbx-13 close; the tally is quoted verbatim in `docs/readout.md`). Run it first
...
readouts, gotchas and history resolve through it). The last closed sitting is **bbx-13** (2026-09-10); the next is **bbx-14**.
...
**Next-session orientation (written at the bbx-13 close, 2026-09-10)**
...
- **S4 step 2 is the next sitting's work** (`docs/plans/S4.md` §8.2, §3 D1–D5, O1, O5, O6): `lib/py/bbx/cli.py` grows into the driver's core — `resolve` over `CLI_PATH` (`bbx.docset resolve`'s shape), the sandbox as the working directory AND `HOME` AND `TMPDIR` (a fresh `mktemp -d` when omitted), the scrub (`env -i` with D6's set plus the scenario's `[env]`, `CLI_KEEP_ENV` REFUSED), `run` (the out file removed first; `argv.txt` / `stdin.bin` / `env.txt` recorded in the sandbox, O5), the log through the vocabulary already in the module (D47) and the band view `<out>.bands`, `summary` (`NOTE: band-fields <n>`, `NOTE: emitted-files <n>`, `NOTE: exit <n>` — then `log_summary` joins the profile, D43 amended), the self-test; `drivers/cli.sh` with D4's four exits (a tool's non-zero exit is an OBSERVATION; a signal death exit 2 with `END-CRASH <n>` and the partial log kept; `REFUSED … cannot honour scenario key 'argz'` exit 3; `CLI_TIMEOUT` — its D-row, arbitrary 60 s — exit 1 DISCARDED); `gates/cli_driver.sh` with §5's seven controls (crash vs exit, the unknown option GREEN and the wrong truth RED, the unknown scenario key, the sandbox both ways, the env scrub with `FAKECLI_SALT` measured direct-vs-driver and through `[env]`, the uncaptured emitted file, the timeout through a wrapper driver). The step's first assertion: the driver's log equals each truth log byte for byte, twice, AND one point re-hashed in `sh` (`printf '%s' "$line" | python3 -m bbx.sha1`) beside `bbx.cli` — the one writer checked by a second implementation (BBX-15).
- The fixture already carries what step 2 needs: `--nondet`, `--crash-at <n>`, `--write-home`, `--sleep <s>`, the salt, `07_emit_file`'s declared file, `05_exit_1` and `06_unknown_option` as green scenarios. Their truth logs are written from the design; step 2 is where they first meet a run.
- Where the built thing differs from the plan, the plan is corrected FIRST in its own commit with a retraction row (BBX-19, as X14–X19 were this sitting).
- The shadow practice stands (`git archive HEAD | tar -x -C <scratch>`, `git init` there, apply, every touched gate THROUGH THE CLASSIFIER, then the new gate in the TREE, then the battery). It caught three defects of the new gate and generator this sitting; the tree caught the fourth (the unstaged fixture, G22's guard) — see the hazards.
