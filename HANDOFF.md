...
| **R21's FIRST RUN, and what it led to** | `docs/platforms/wsl/` — WSL 2026-09-13: the battery went NOT GREEN because a skipped gate could not fire its declared controls and the reader counted them dead (G47). Fixed at bbx-22 by R48 (`8d5f97d`): a gate the classifier calls SKIP reads `verdict=SKIPPED`, its declarations set aside and named, and `--strict` refuses every skip. Everything else was identical to macOS. The run file is a WITNESS (the kept directory is not in the tree); **R21 ANSWERED after the bbx-22 close; the re-run at the fix reported GREEN (three pasted lines, a witness), and the platform row waits on the kept pair** |
...
- **R21 is answered** (after the bbx-22 close): the maintainer runs the platform procedure and
  brings back the kept pair; a platform reads green only on such a pair at a commit whose macOS
  battery is also green. The WSL re-run at the fix reported `PASS 30  SKIP 1 … GREEN` — three
  pasted lines, a WITNESS. **When the archive arrives:** commit the kept pair under
  `docs/platforms/wsl/`, run `bin/bbx readout <second> --against <first>` on it, and check that it
  names a commit at `429d3f8` or later, `verdict=SKIPPED` for `census_recount`, `skipped: 1, whose 4
  declared control(s) assert nothing`, and BBX-14 met. That turns the platform row green. Anything
  red in it is the finding, and BBX-26 applies to it. The maintainer's rider — SSH or basic
  automation for these runs — becomes a ruling once the cadence of platform runs is known.
