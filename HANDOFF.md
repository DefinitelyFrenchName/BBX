...
takes **about 10 to 15 min on this host** (32 registered gates; the gate runtimes of bbx-25's four kept runs summed 859, 751, 624 and 616 s, re-derived from their `results.tsv`),
...
1. **Green first, twice, kept.** Run the battery alone, twice, each with
   ...
   with 32 registered gates, past the foreground cap); BBX-14 needs two runs at one HEAD;
...
- Open with the battery **in the background** (10 to 15 min, 32 registered gates), with
  `--log build/selftest_<stamp>`, alone, nothing edited while it runs.
- **Expect NO `census-drift` NOTE** at `e81417d` or at any docs-only descendant: the census records
  identity `c26e5a94b344`. A commit touching `bin`, `lib`, `drivers` or `gates` brings the NOTE back,
  loud and never fatal (R47).
- **Expect `header_entries=249 continued=0`** on the close sweep's PASS line, and controls fired 133 /
  declared 133 (`readout` declares 12). A gate or driver added with entries moves the first number; an
