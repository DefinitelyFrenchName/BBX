...
bin/bbx selftest` (~8.5 min on this host) is GREEN twice at one HEAD at the bbx-17 close: VERDICT: GREEN   PASS 28  SKIP 0  FAIL 0  TIMEOUT 0  MISSING 0   (gates 28), 105/105
controls (`build/selftest_20260911T194343Z`, `build/selftest_20260911T200013Z`; the tally is quoted verbatim in
`docs/readout.md`'s bbx-17 CLOSE). Run it first thing, with `--log build/selftest_<stamp>` and no edits in flight
(the runner's working-tree check reports a concurrent edit as DIRTIED); the census recount inside it is the
...
readouts, gotchas and history resolve through it). The last closed sitting is **bbx-17** (2026-09-11); the next is **bbx-18**.
...
**Next-session orientation (written at the bbx-17 close, 2026-09-11)**
- Open first: `bin/bbx selftest --log build/selftest_<stamp>` and `bin/bbx readout` on it. Two `drift` NOTEs are expected: VampireSaved (36 past its census) and bbh (`1af19c7`, 6 past the baseline `10a82d2` — it moves often, having gone 447e5d2 → 02d58f3 → 777dbde → 1af19c7 across bbx-16 and bbx-17, so re-read the NOTE rather than this line; follow it by the R28 procedure only when a sitting needs the current tip). The battery is ~8.5 min: one battery per command under a 10-minute cap.
- **bbx-17 built no step.** It answered R41 (both writers of `band-fields` kept, against the recommendation) and R42 (the fixture's deliberate death changed from `os.abort()` to SIGKILL, because macOS filed a crash report in the maintainer's home for every one — 25 of them over two days, and the MAINTAINER was the detector, not a gate: G30). S4 step 5 is untouched and is the next sitting's work.
- **S4 step 5 is the next sitting's work** (`docs/plans/S4.md` §8.5, §3 "D7" and "R14", §5's adapter rows, §7's `derived` class; R37, R38 answered at bbx-12): `drivers/unittest.sh` and `drivers/gates.sh` over the same `bbx.cli` core (the sandbox, the scrub, the timeout, the log writer — three consumers of the core at the end of the step, BBX-25 with one to spare); `fixture/unittest/` (a self-contained unittest package with a designed failing test; the path corrected at bbx-18) and `fixture/selfgates/` (a synthetic `self`-kind consumer whose scenarios drive `bbx run-static` through the dispatcher — two of the census's "+kernel" rows; the sweep runner is NOT a subject of this adapter, which maps a kept `results.tsv` that `--list` never writes, measured bbx-18); the identity of the self subject is R38's `command` key (`git rev-parse HEAD:bin HEAD:lib HEAD:drivers`); `gates/adapters.sh` (the framework's verdict as an OBSERVATION — a failing test is `case:FAIL:…` and PASSes against a truth that expects it; `Ran 0 tests` exit 1 DISCARDED; the self subject both ways; a framework run twice that differs → NONDETERMINISTIC). The adapters' kinds table is the command-line profile's (the view column: `truth` → `log`); the new token kinds `case` and `gate` are in the PLAN's §7 list but were NOT in `docs/defaults.md`'s D47 row, which carries five (measured at bbx-18; the row is extended as the step lands) — measure that `bbx.cli`'s splitter accepts them before the first log is written. Expectations `derived` class, never `fixture` (§3 E2, E3).
- **A test framework that ABORTS is the thing to watch at step 5** (R42, G30): a unittest process that dies by a fault signal — and a framework that segfaults on a bad test is ordinary — files a host crash report the driver never removes. Count `ls ~/Library/Logs/DiagnosticReports/*.ips | wc -l` before and after the new gate's first run and expect zero growth; if a control NEEDS a fault signal, say so in the ruling rather than letting the reports accumulate silently.
- **First small fix, still open from bbx-16 (G28):** `gates/docset_suite.sh` and `gates/cli_suite.sh` copy the kept run (`cp -R "$LOGDIR" "$R1"`) right after the positive run; a suite that never created it ends the gate under `set -e` with a `cp:` line and no FAIL of its own. Guard `[ -d "$LOGDIR" ]` and print the suite's output as the FAIL line. Two lines per gate, both gates re-run through the classifier. bbx-17 did not do it: the sitting was spent on two rulings.
...
- Where the built thing differs from the plan, the plan is corrected FIRST in its own commit with a retraction row (BBX-19): bbx-17 did it for X25 (`9f16238`), as bbx-15 and bbx-16 did.
- The shadow practice stands (`git ls-files | tar` from the WORKING tree — bbx-17 used the working tree rather than `git archive HEAD`, so the shadow carried the sitting's uncommitted document corrections and its `close_sweeps` was meaningful; then `git init` there, apply, every touched gate THROUGH THE CLASSIFIER, then every NEIGHBOURING gate (G28: the neighbour was the detector), then the whole portable tier there, then the new gate in the TREE, then the battery). A `--log` path given to the suite is resolved against the CONSUMER's root, not the caller's cwd: pass it absolute.
- G19's learning (a rot gate over generated and printed text) has instances G24, G27 and G29 and is S6's; R29 (executable controls) is built in S6; G30's crash-report counter is a new S6 candidate. R21 is still open and is the ONLY open ruling.
