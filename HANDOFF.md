...
takes **about 10 to 15 min on this host** (33 registered gates since bbx-26; the gate runtimes of bbx-25's four kept 32-gate runs summed 859, 751, 624 and 616 s, re-derived from their `results.tsv`),
...
1. **Green first, twice, kept.** Run the battery alone, twice, each with
   ...
   with 33 registered gates, past the foreground cap); BBX-14 needs two runs at one HEAD;
...
- Open with the battery **in the background** (10 to 15 min, 33 registered gates), with
  `--log build/selftest_<stamp>`, alone, nothing edited while it runs.
- **Expect NO `census-drift` NOTE** at `e285c4d` or at any docs-only descendant: the census records
  identity `f189534d8e9e` (regenerated at bbx-26, `ada1d15`, 47 rows). A commit touching `bin`, `lib`, `drivers` or `gates` brings the NOTE back,
  loud and never fatal (R47).
- **Expect `header_entries=255 continued=0`** on the close sweep's PASS line, and controls fired 134 /
  declared 134 (`readout` declares 12; measured over the S5 step 1 build at bbx-26). A gate or driver added with entries moves the first number; an
