...
| **slice S4's plan** (the ancestors measured, the design contract by contract, the fixture `fixture/fakecli/`, eighteen controls, the adapters, the self subject, the file census as a gate, rulings R35–R40) | `docs/plans/S4.md` — bbx-12; **R35–R40 answered 2026-09-10** (R40 with the TSV caveat, applied in the plan's status line); S3 ruled DONE the same day; **step 1 may open** |
...
- **Both rulings are in (2026-09-10, after the bbx-12 close):** S3 ruled DONE; R35–R40 answered — R35–R39 as recommended, R40 with the maintainer's caveat "no caveat on R40 but a caveat on the use of TSV elsewhere" (applied: every file S4 introduces is a TOML-subset file (the `.cli` scenario, the `unordered`, `schema`, `band` expectations, `expected/file_census.toml`); the kernel's pre-existing TSVs (`results.tsv` D30, `notes.tsv` D44, `registry.tsv`, `gates/sweep.tsv`) are not multiplied, and the gates adapter reads `results.tsv` by column name only, never the printed rows). The generality page's crash row is already corrected (X13). **S4 step 1 is the next sitting's work** (`docs/plans/S4.md` §8.1: the `command-line` profile, `scenario_ext = "cli"`, the kinds rows, `mkfakecli.py`, the fixture under `fixture/fakecli/`, its `bbx.toml`, `gates/cli_fixture.sh`) — with a new expectation tree under a new path, its `.gitignore` exception BEFORE the first freeze (G22). The plan's design targets to measure at S4's close: the kinds loop changed by zero lines; the three "+kernel" files of the bbx-11 census (`bin/bbx`, `bin/bbx-run-static`, `bin/bbx-run-sweep`) executed by gates of two kinds; six shared files by three; unreached 0.
- Where the built thing differs from the plan, the plan is corrected FIRST in its own commit with a retraction row (BBX-19, as S3's X10–X12 were), then the readout quotes it.
