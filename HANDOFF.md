...
| **slice S4's plan** (the ancestors measured, the design contract by contract, the fixture `fixture/fakecli/`, eighteen controls, the adapters, the self subject, the file census as a gate, rulings R35–R40) | `docs/plans/S4.md` — bbx-12; R35–R40 answered 2026-09-10 (R40 with the TSV caveat); **steps 1, 2 and 3 built (bbx-13, bbx-14, bbx-15)**; §3/§4 corrected at bbx-13 before step 1 landed (X14–X19, BBX-19); step 2 needed no correction; §2/§3/§5 corrected at bbx-15 before step 3 landed (X20, X21); **step 4 is next** |
...
| BBX's gates and registries | `gates/*.sh` (27: 23 portable incl. `temporal`, `thresholds`, `compare_dispatch`, `expectation_kinds`, `provenance`, `docset_fixture`, `docset_driver`, `set_schema`, `docset_suite`, `cli_fixture`, `cli_driver`, `band`, `json_schema`; 4 static incl. `fidelity_bbh_s2` (F12, F16, F17) and `suite`), `gates/portable.txt`, `gates/static.txt`, `gates/sweep.tsv` (empty) — run with `BBX_BBH_HOME=~/Developer/blackbox-harness bin/bbx selftest` (~6 min on a loaded host: the two S2 static gates are ~100 s and ~60 s, `docset_suite` ~55 s) |
...
bin/bbx selftest` (~7 min on this host) is GREEN twice at one HEAD at the bbx-15 close: 27 gates, 98/98 controls (`build/selftest_20260910T193009Z`, `build/selftest_20260910T193718Z`; the tally is quoted verbatim in `docs/readout.md`'s bbx-15 CLOSE). Run it first
...
readouts, gotchas and history resolve through it). The last closed sitting is **bbx-15** (2026-09-10); the next is **bbx-16**.
...
**Next-session orientation (written at the bbx-15 close, 2026-09-10)**
...
- **S4 step 4 is the next sitting's work** (`docs/plans/S4.md` §8.4, §3 "E1" under "Under this profile the loop…", §5's shadow-tool, `--nondet`, `--crash-at`, unknown-option and JSON rows): the suite over `fixture/fakecli/` — `bin/bbx run-suite --config fixture/fakecli/bbx.toml --log <dir>` twice, the identity (the whole-set key over `subject/`), the kinds loop over `truth` / `unordered` / `schema` / `band`, `NOTE: band-fields` and `NOTE: exit` on the screen through `log_summary` (D43), RUN-FAIL on a crash, NONDETERMINISTIC three ways; `gates/cli_suite.sh`; the readout's band note.
- **Its first decision, measured at bbx-15 and not yet made:** the kinds loop hands the schema family the SUBJECT FILE (`bbx.fingerprint --path`, `bin/bbx-run-suite` line 245's `$SUBJECT_FILE`), which for the json format is the wrong artifact — `gates/json_schema.sh` proves it: `FAIL schema: the artifact is not one JSON object`. The json format's artifact is the driver's JSON view `<log>.json` (D54). So the plan's zero-line target for the loop (§3 E1) will not survive as written, and the plan is corrected FIRST with the measured count (BBX-19; done at bbx-16, X22). Recommendation: declare the VIEW a kind reads in the kinds table (BBX-16 — every comparator declares its view; a fourth column, `artifact` for the document-set schema and `json` for the command-line one, `bands` for the band kind), the loop resolving a view name to a path in one place; the alternative — `compare.sh`'s schema branch deriving the artifact from the spec's `format` — touches an S3 function the fidelity of which is `gates/set_schema.sh`, and hides a kind fact inside a family. Either way one measured line count, and `expectations.py`'s table shape is a D-row amendment (D45's kinds row).
- Under `--freeze` the band kind is AUTHORED (`authored .band expectation — not self-frozen`, the loop's existing line): `compare_band --freeze` is the maintainer's tool over a proposed file (D55), never the suite's. The `unordered` kind is authored too (inventory mode). Only a scenario with no authored kind self-freezes — `04_band` and `02_unordered` and `03_show_json` all carry one; the six `truth`-only scenarios carry `truth`, also authored: NOTHING in this fixture self-freezes, which is the design (every expectation `fixture` class, written from the design).
- Where the built thing differs from the plan, the plan is corrected FIRST in its own commit with a retraction row (BBX-19): bbx-15 did it for X20 and X21 (`9567d7c`).
- The shadow practice stands (`git archive HEAD | tar -x -C <scratch>`, `git init` there, apply, every touched gate THROUGH THE CLASSIFIER, then the whole portable tier there with `bin/bbx selftest` without `BBX_BBH_HOME`, then the new gate in the TREE, then the battery). Write the new `docs/defaults.md` rows in the shadow too (bbx-15 did: the shadow's `close_sweeps` PASS first time).
- G19's learning (a rot gate over generated and printed text for "until slice N" / "is step N" sentences) has two instances (G24) and is S6's; G27 adds a third candidate to that queue (the close-commit message's sweep tuple against the kept log); R29 (executable controls) is built in S6; R21 is still open.
