...
| **slice S4's plan** (the ancestors measured, the design contract by contract, the fixture `fixture/fakecli/`, eighteen controls, the adapters, the self subject, the file census as a gate, rulings R35–R40) | `docs/plans/S4.md` — bbx-12; R35–R40 answered 2026-09-10 (R40 with the TSV caveat); **steps 1 and 2 built (bbx-13, bbx-14)**; §3/§4 corrected at bbx-13 before step 1 landed (X14–X19, BBX-19); step 2 needed no correction; **step 3 is next** |
...
| BBX's gates and registries | `gates/*.sh` (25: 21 portable incl. `temporal`, `thresholds`, `compare_dispatch`, `expectation_kinds`, `provenance`, `docset_fixture`, `docset_driver`, `set_schema`, `docset_suite`, `cli_fixture`, `cli_driver`; 4 static incl. `fidelity_bbh_s2` (F12, F16, F17) and `suite`), `gates/portable.txt`, `gates/static.txt`, `gates/sweep.tsv` (empty) — run with `BBX_BBH_HOME=~/Developer/blackbox-harness bin/bbx selftest` (~6 min on a loaded host: the two S2 static gates are ~100 s and ~60 s, `docset_suite` ~55 s) |
...
bin/bbx selftest` (~6–7 min on this host) is GREEN twice at one HEAD at the bbx-14 close: 25 gates, 89/89 controls (`build/selftest_20260910T180335Z`, `build/selftest_20260910T181015Z`; the tally is quoted verbatim in `docs/readout.md`'s bbx-14 CLOSE). Run it first
...
readouts, gotchas and history resolve through it). The last closed sitting is **bbx-14** (2026-09-10); the next is **bbx-15**.
...
**Next-session orientation (written at the bbx-14 close, 2026-09-10)**
...
- **S4 step 3 is the next sitting's work** (`docs/plans/S4.md` §8.3, §3 "E1", "C1–C3"; §5's band, schema and unordered rows): `lib/py/bbx/compare_band.py` over `<log>.bands` (D53's view: `<i> <name>=<value>`, `END <k>`; the spec D48: inclusive integer bands, `measured`, the optional `rulings` inline table; `FAIL band <field>: <v> outside [<min>, <max>]` naming all three; `NOTE: band-fields <n>`; `--freeze` REFUSING to widen a band whose field has no `rulings` entry, exit 3, the file unchanged — narrowing free); the `tolerant-numeric)` branch of `lib/sh/compare.sh` (R23: family by kind; the three existing branches change by zero lines, measured); the `json` format in `lib/py/bbx/compare_schema.py` (D49: keys in sorted order, one type each from R40's seven, `items_op`/`items_n` for a list; the FIRST violation named; `FAIL unknown schema type`; D41's TSV vocabulary untouched) reading the driver's log through `bbx.cli split_token` — and note the schema is judged on the OBJECT, so step 3 decides where the comparator gets it: the kept run's log holds hashes only, so the driver must keep the object's shape somewhere the comparator can read (a `<out>.json` view beside the band view, the shape of D53, is the recommendation; a D-row); `compare_set.py`'s inventory mode over `02_unordered.unordered` (`[l<i>] line, sha1` rows, both ways; a duplicate named as hand-editing); `gates/band.sh` and `gates/json_schema.sh` with §5's controls (the ruling-less class; the shadow tool renaming `weight` → schema FAIL and `NOT-EVALUATED`; the value changed → schema PASS, truth FAIL; the inventory both ways; the reversed-order truth). Every verdict line frozen from the first green (C4) and classified by `finding.py`, which gains no word (measured by the gate).
- Where the built thing differs from the plan, the plan is corrected FIRST in its own commit with a retraction row (BBX-19). Step 2 needed none; step 3's open point above (where the JSON object lives for the schema comparator) is a plan silence, not a plan error — decide it in a D-row and say so in the readout.
- The shadow practice stands (`git archive HEAD | tar -x -C <scratch>`, `git init` there, apply, every touched gate THROUGH THE CLASSIFIER, then the whole portable tier there with `bin/bbx run-static`, then the new gate in the TREE, then the battery). Write the new `docs/defaults.md` rows in the shadow too (the hazard below).
