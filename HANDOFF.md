...
- **Its first decision, measured at bbx-15 and not yet made:** the kinds loop hands the schema family the SUBJECT FILE (`bbx.fingerprint --path`, `bin/bbx-run-suite` line 245's `$SUBJECT_FILE`), which for the json format is the wrong artifact — `gates/json_schema.sh` proves it: `FAIL schema: the artifact is not one JSON object`. The json format's artifact is the driver's JSON view `<log>.json` (D54). So the plan's "the loop changes by ZERO lines" (§3 E1) will not survive as written, and the plan is corrected FIRST with the measured count (BBX-19). Recommendation: declare the VIEW a kind reads in the kinds table (BBX-16 — every comparator declares its view; a fourth column, `artifact` for the document-set schema and `json` for the command-line one, `bands` for the band kind), the loop resolving a view name to a path in one place; the alternative — `compare.sh`'s schema branch deriving the artifact from the spec's `format` — touches an S3 function the fidelity of which is `gates/set_schema.sh`, and hides a kind fact inside a family. Either way one measured line count, and `expectations.py`'s table shape is a D-row amendment (D45's kinds row).
